...
import { ADDRESS_PREFIX, JIO_UNIT } from "../explorer_constants";
                ...
                <Col sm={6} md={4}>
                    ...
                    <div className="utxo-value d-flex">
                        {addressBalance !== undefined ?
                            <div className="utxo-amount">+{numberWithCommas(addressBalance / 100000000)} {JIO_UNIT}</div> :
                            ...
                            <div className="utxo-value">
                                <Link className="blockinfo-link" to={`/txs/${x.transaction_id}`}>
                                    {getAmount(x.outputs, x.inputs) > 0 ?
                                        <span
                                            className="utxo-amount">+{numberWithCommas(floatToStr(getAmount(x.outputs, x.inputs)))}&nbsp;{JIO_UNIT}</span> :
                                        <span
                                            className="utxo-amount-minus">{numberWithCommas(floatToStr(getAmount(x.outputs, x.inputs)))}&nbsp;{JIO_UNIT}</span>}
                                </Link>
                            </div>
                        </Col>
                        <Col sm={6} md={2}>
                                    ...
                                    {x.inputs?.length > 0 ? x.inputs.map(x => {
                                        return (txsInpCache && txsInpCache[x.previous_outpoint_hash]) ? <>
                                            <Row id={`N${x.previous_outpoint_hash}${x.previous_outpoint_index}`}>
                                                ...
                                                <Col xs={5}><span
                                                    className="block-utxo-amount-minus">-{numberWithCommas(getAmountFromOutputs(txsInpCache[x.previous_outpoint_hash]["outputs"], x.previous_outpoint_index))}&nbsp;{JIO_UNIT}</span></Col></Row></> :
                            ...
                            <Col sm={12} md={6}>
                                ...
                                <div className="utxo-value-mono" style={{ fontSize: "smaller" }}>
                                    {x.outputs.map(x => <Row>
                                        ...
                                        <Col xs={5}><span
                                            className="block-utxo-amount">+{numberWithCommas(x.amount / 100000000)}&nbsp;{JIO_UNIT}</span></Col></Row>)}
                                </div>
                            </Col>
                            <Col md={12}>
                    ...
                    <Row className="utxo-border pb-4 mb-4">
                        ...
                        <Col sm={6} md={4}>
                            ...
                            <div className="utxo-value d-flex flex-row">
                                <div className="utxo-amount">+{numberWithCommas(x.utxoEntry.amount / 100000000)} {JIO_UNIT}
                                </div>
                            </div>
                        </Col>
                        <Col sm={6} md={4}>
