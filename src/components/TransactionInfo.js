...
import { getTransaction, getTransactions } from '../jio-api-client.js';
                    ...
                    {!!txInfo && txInfo?.detail !== "Transaction not found" ?
                        <div className="blockinfo-content">
                            ...
                            <Container className="blockinfo-table mx-0" fluid>
                                ...
                                {showTxFee && txInfo.inputs && txInfo.inputs.length > 0 && <Row className="blockinfo-row">
                                    <Col className="blockinfo-key" lg={2}>Transaction fee</Col>
                                    <Col className="blockinfo-value-mono" lg={10}>
                                        {txInfo && additionalTxInfo &&
                                            <>{(txInfo.inputs.map((tx_input) => (getOutputFromIndex(additionalTxInfo[tx_input.previous_outpoint_hash]?.outputs || [], tx_input.previous_outpoint_index)?.amount || 0)).reduce((a, b) => a + b) - (txInfo.outputs?.map((v) => v.amount) || [0]).reduce((a, b) => a + b)) / 100000000} {JIO_UNIT}</>
                                        }
                                    </Col>
                                </Row>}
                                                    ...
                                                    <div className="utxo-value">
                                                        <span
                                                            className="utxo-amount-minus">-{numberWithCommas(getOutputFromIndex(additionalTxInfo[tx_input.previous_outpoint_hash]
                                                                .outputs, tx_input.previous_outpoint_index).amount / 100000000)}&nbsp;{JIO_UNIT}</span>
                                                    </div>
                                                </Col>}
                                    ...
                                    (txInfo.outputs || []).map((tx_output) =>
                                        <Row className="utxo-border py-3">
                                            ...
                                            <Col sm={6} md={6} lg={3}>
                                                ...
                                                <div className="utxo-value">
                                                    <span
                                                        className="utxo-amount">+{numberWithCommas(tx_output.amount / 100000000)}&nbsp;{JIO_UNIT}</span>
                                                </div>
                                            </Col>
                                            <Col sm={12} md={12} lg={12}>
