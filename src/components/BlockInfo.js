...
import { JIO_UNIT } from "../explorer_constants.js";
import { numberWithCommas } from "../helper.js";
import { getBlock, getTransactions } from '../jio-api-client.js';
...
const sompiOrKas = (inp) => {
    ...
    if (parseFloat(inp.toFixed(8)) < 0.00001) {
        ...
        return `${parseFloat(inp.toFixed(8))} {JIO_UNIT}`
    }
}
                            ...
                            <Container className="blockinfo-table mx-0" fluid>
                                <Row className="blockinfo-row">
                                    ...
                                    <Col className="blockinfo-value-mono" lg={10}>{blockInfo.verboseData.hash}
                                        ...
                                        <OverlayTrigger
                                            overlay={<Tooltip id="tooltip-kgi">Open in Jio Graph Inspector</Tooltip>}>
                                            <span>
                                                <BiNetworkChart className="ms-2 copy-symbol" size="20" onClick={() => {
                                                    window.open(`https://kgi.jiopad.net/?hash=${id}`, '_blank');
                                                }} />
                                            </span>
                                        </OverlayTrigger>
                                    </Col>
                                        ...
                                        <Row className="utxo-border py-3">
                                                        ...
                                                        {(tx.inputs || []).map((txInput) => <Row>
                                                            {!!txInfo && txInfo[txInput.previousOutpoint.transactionId] ? <>
                                                                <Col xs={12} sm={8} md={9} lg={9} xl={8} xxl={7}
                                                                    ...
                                                                    sm={4} md={2}>
                                                                    -{numberWithCommas(getAmountFromOutputs(txInfo[txInput.previousOutpoint.transactionId]["outputs"], txInput.previousOutpoint.index || 0))}&nbsp;{JIO_UNIT}
                                                                </Col></>
                                                                :
                                                                <><Col xs={12} sm={8} md={9} lg={9} xl={8} xxl={7}
                                                                    className="text-truncate">
                                                                    <a className="blockinfo-link"
                                                                        href={`https://katnip.jiopad.net/tx/${txInput.previousOutpoint.transactionId}`}
                                                                        target="_blank">
                                                                        TX
                                                    ...
                                                    <Container className="utxo-value-mono" fluid>
                                                        {(tx.outputs || []).map((txOutput) => <Row>
                                                            <Col xs={12} sm={8} md={9} lg={9} xl={8} xxl={7}
                                                                ...
                                                                md={3}>+{numberWithCommas(txOutput.amount / 100000000)}&nbsp;{JIO_UNIT}</Col>
                                                        </Row>)}
                                                    </Container>
                                                </Col>
                                            </Col>
                                            <Col sm={5} md={4} lg={3}>
                                                ...
                                                <div className="utxo-value d-flex flex-row">
                                                    <div
                                                        className="utxo-amount">{(numberWithCommas(tx.outputs.reduce((a, b) => (a || 0) + parseInt(b.amount), 0) / 100000000))} {JIO_UNIT}
                                                    </div>
                                                </div>
                                            </Col>
                                            {/*<Col sm={3} md={2}>*/}
